...
def main():
    ...
    tables_tag = (
        f"sigma_{artifact_sigma_tag(sigma)}"
        f"_det{int(round(determinism * 100)):02d}"
    )
    heatmap_suffix = f"{args.lq_source}_{tables_tag}"
    heatmaps = generate_pruning_heatmap(
        q_single,
        bounds,
        lipschitz,
        output_dir / f"pruning_heatmap_{heatmap_suffix}",
    ...
    if args.lq_source == "theoretical":
        table1 = write_table1(lipschitz, output_dir / f"table1_lipschitz_constants_{tables_tag}.csv")
        table2 = write_table2(bounds, output_dir / f"table2_transition_bounds_{tables_tag}.csv")
        saved.extend([table1, table2])
    print(
        f"runtime dynamics sigma={sigma:.10g}; "
