...
def add_environment_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Noise std of category 5 as a multiple of sigma (overrides "
            "config.environment.deterministic_sigma_scale). 0 = exactly deterministic, "
            "0.1 = 10x quieter than sigma, 0.01 = 100x quieter."
        ),
    )
    parser.add_argument(
        "--stochasticity-scale",
...
def resolve_determinism(config: Dict[str, Any], determinism: Optional[float]) -> float:
    from dollar_euro_lipschitz.layout import clamp_determinism

    if determinism is not None:
        return clamp_determinism(determinism)
    return clamp_determinism(float(config.get("environment", {}).get("determinism", 0.25)))


def apply_resolved_determinism(config: Dict[str, Any], determinism: float) -> Dict[str, Any]:
    environment = dict(config.get("environment") or {})
    environment["determinism"] = float(determinism)
    config["environment"] = environment
    try:
        from dollar_euro_lipschitz.bounds import set_default_determinism
...
def resolve_deterministic_sigma_scale(
    config: Dict[str, Any],
    deterministic_sigma_scale: Optional[float],
) -> float:
    if deterministic_sigma_scale is not None:
        value = float(deterministic_sigma_scale)
    else:
        value = float(
            config.get("environment", {}).get("deterministic_sigma_scale", 0.0)
        )
    if value < 0.0:
        raise ValueError("deterministic_sigma_scale must be >= 0.")
    return value


def apply_resolved_deterministic_sigma_scale(
    config: Dict[str, Any], deterministic_sigma_scale: float
) -> Dict[str, Any]:
    environment = dict(config.get("environment") or {})
    environment["deterministic_sigma_scale"] = float(deterministic_sigma_scale)
    config["environment"] = environment
    return config
...
def env_kwargs(config: Dict[str, Any], sigma: float) -> Dict[str, Any]:
    ...
    return {
        "sigma": float(sigma),
        "determinism": float(environment.get("determinism", 0.25)),
        "deterministic_sigma_scale": float(
            environment.get("deterministic_sigma_scale", 0.0)
        ),
        "step_size": float(environment.get("step_size", 0.04)),
        "tau": float(environment.get("tau", 4.4)),
        "reward_alpha": float(environment.get("reward_alpha", 2.0)),
        "reward_beta": float(environment.get("reward_beta", 1.2)),
