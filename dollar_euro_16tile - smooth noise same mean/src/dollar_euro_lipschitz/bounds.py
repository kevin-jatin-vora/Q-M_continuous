...
# State transitions are recorded as float32.  Subtracting two coordinates near
# one another leaves ~1e-8 representation noise even when the configured
# displacement is exactly deterministic.  Quantize displacement summary
# statistics uniformly across all experiments before constructing a radius.
TRANSITION_STAT_DECIMALS = 7

# Module default for heatmap / pruning callers that omit determinism.
_DEFAULT_DETERMINISM = 0.25


def quantize_transition_stat(values, decimals: int = TRANSITION_STAT_DECIMALS) -> np.ndarray:
    """Round transition statistics to the project's fixed decimal precision.

    This is intentionally configuration-independent: deterministic and
    stochastic runs use exactly the same quantization rule.
    """
    quantized = np.round(np.asarray(values, dtype=np.float64), decimals=int(decimals))
    # Normalize negative zero so JSON artifacts and exact-zero checks agree.
    return np.asarray(np.where(quantized == 0.0, 0.0, quantized), dtype=np.float64)
