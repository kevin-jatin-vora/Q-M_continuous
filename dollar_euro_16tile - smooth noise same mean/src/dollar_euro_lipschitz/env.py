...
Dynamics: s' = clip(s + Delta(a) + sigma(s) Z, 0, 1), Z ~ N(0, I_2).
The action displacement is identical in every category.  Category-specific
scalar noise targets are bilinearly interpolated between tile centers, so the
transition kernel is continuous across terrain boundaries.  The outer boundary
uses direct clipping, which is non-expansive.
"""


NOISE_MODEL_VERSION = "smooth_scalar_bilinear_v1"
CATEGORY_NOISE_MULTIPLIERS = {1: 1.0, 2: 1.1, 3: 1.2, 4: 1.3}


def noise_multiplier_table(deterministic_sigma_scale: float) -> np.ndarray:
    """Category-indexed scalar noise multipliers (slot zero is unused)."""
    values = np.zeros(N_CATEGORIES + 1, dtype=np.float64)
    for category, multiplier in CATEGORY_NOISE_MULTIPLIERS.items():
        values[int(category)] = float(multiplier)
    values[DETERMINISTIC_CATEGORY] = float(deterministic_sigma_scale)
    return values


def tile_noise_multiplier_grid(tile_map, deterministic_sigma_scale: float) -> np.ndarray:
    mapping = np.asarray(tile_map, dtype=np.int32).reshape(GRID_SIZE, GRID_SIZE)
    return noise_multiplier_table(deterministic_sigma_scale)[mapping]


def _interpolation_coordinate(value: float):
    """Return lower center index and interpolation fraction with edge clamping."""
    ...
    if coordinate <= first:
        return 0, 0.0
    if coordinate >= last:
        return GRID_SIZE - 2, 1.0
    scaled = (coordinate - first) / spacing
    lower = min(GRID_SIZE - 2, max(0, int(np.floor(scaled))))
    return lower, float(scaled - lower)


def interpolate_noise_multiplier(state, multiplier_grid) -> float:
    """Bilinear terrain noise multiplier evaluated at a continuous state."""
    grid = np.asarray(multiplier_grid, dtype=np.float64).reshape(GRID_SIZE, GRID_SIZE)
    ix, tx = _interpolation_coordinate(float(state[0]))
    iy, ty = _interpolation_coordinate(float(state[1]))
    q00, q10 = grid[iy, ix], grid[iy, ix + 1]
    q01, q11 = grid[iy + 1, ix], grid[iy + 1, ix + 1]
    bottom = (1.0 - tx) * q00 + tx * q10
    top = (1.0 - tx) * q01 + tx * q11
    return float((1.0 - ty) * bottom + ty * top)


def noise_multiplier_lipschitz(multiplier_grid) -> float:
    """Exact global gradient-norm maximum of the clamped bilinear field."""
    grid = np.asarray(multiplier_grid, dtype=np.float64).reshape(GRID_SIZE, GRID_SIZE)
    spacing = 1.0 / GRID_SIZE
    maximum = 0.0
    for iy in range(GRID_SIZE - 1):
        for ix in range(GRID_SIZE - 1):
            q00, q10 = grid[iy, ix], grid[iy, ix + 1]
            q01, q11 = grid[iy + 1, ix], grid[iy + 1, ix + 1]
            # Each derivative is affine in the other coordinate.  The squared
            # gradient norm is separately convex, so its cell maximum is at a corner.
            for tx in (0.0, 1.0):
                for ty in (0.0, 1.0):
                    dx = ((1.0 - ty) * (q10 - q00) + ty * (q11 - q01)) / spacing
                    dy = ((1.0 - tx) * (q01 - q00) + tx * (q11 - q10)) / spacing
                    maximum = max(maximum, float(np.hypot(dx, dy)))
    return maximum


def smooth_noise_metadata(sigma: float, determinism: float, deterministic_sigma_scale: float):
    """Serializable noise-field and Wasserstein contractivity metadata."""
    mapping = tile_category_map(float(determinism))
    grid = tile_noise_multiplier_grid(mapping, deterministic_sigma_scale)
    multiplier_lipschitz = noise_multiplier_lipschitz(grid)
    sigma_lipschitz = abs(float(sigma)) * multiplier_lipschitz
    kernel_lipschitz = float(np.sqrt(1.0 + 2.0 * sigma_lipschitz ** 2))
    return {
        "noise_model_version": NOISE_MODEL_VERSION,
        "noise_interpolation": "bilinear_between_tile_centers_constant_edge_extension",
        "category_noise_multipliers": {
            "1": 1.0, "2": 1.1, "3": 1.2, "4": 1.3,
            "5": float(deterministic_sigma_scale),
        },
        "tile_noise_multiplier_grid_row_major": grid.tolist(),
        "noise_multiplier_lipschitz": multiplier_lipschitz,
        "sigma_lipschitz": sigma_lipschitz,
        "wasserstein_kernel_lipschitz": kernel_lipschitz,
    }
...
def _category_templates(base_sigma: float, deterministic_sigma_scale: float = 0.0) -> Dict[int, Dict[str, Any]]:
    """Category metadata at tile centers for the smooth scalar-noise field."""
    multipliers = noise_multiplier_table(deterministic_sigma_scale)

    def template(category):
        target_sigma = float(base_sigma) * float(multipliers[category])
        return {
            "name": CATEGORY_NAMES[category],
            "action_scale": np.ones(2, dtype=np.float32),
            "drift": np.zeros(2, dtype=np.float32),
            "noise_multiplier": float(multipliers[category]),
            "noise_cov": np.eye(2, dtype=np.float64) * target_sigma ** 2,
        }

    return {
        category: template(category) for category in range(1, N_CATEGORIES + 1)
    }
...
class ContinuousDollarEuroEnv(gym.Env):
    ...
    def __init__(
        ...
        *,
        ...
        categories = _category_templates(base_sigma, det_scale)
        self.noise_metadata = smooth_noise_metadata(base_sigma, self.determinism, det_scale)
        self.terrain_config: Dict[str, Any] = {
            ...
            "regions": categories,
            "smooth_noise": self.noise_metadata,
        }
    ...
    def _cache_vectorized_tables(self):
        ...
        self._noise_multiplier_grid = np.asarray(
            self.noise_metadata["tile_noise_multiplier_grid_row_major"], dtype=np.float64
        )
    ...
    def step(self, action: int):
        ...
        boundary_cancel = False
        local_sigma = self.noise_sigma_at(prev_state)
        noise = local_sigma * self._np_random.standard_normal(2)
        ...
        info = {
            ...
            "noise_sigma": local_sigma,
            "noise_multiplier": self.noise_multiplier_at(prev_state),
        }
    ...
    def noise_multiplier_at(self, state) -> float:
        return interpolate_noise_multiplier(state, self._noise_multiplier_grid)

    def noise_sigma_at(self, state) -> float:
        return float(self.sigma) * self.noise_multiplier_at(state)

    def wasserstein_kernel_lipschitz(self) -> float:
        return float(self.noise_metadata["wasserstein_kernel_lipschitz"])

    def validate_wasserstein_contractivity(self, gamma: float) -> float:
        value = float(gamma) * self.wasserstein_kernel_lipschitz()
        if value >= 1.0:
            raise ValueError(
                "Smooth-noise transition kernel is not contractive: "
                f"gamma*K_P={value:.17g} >= 1 "
                f"(gamma={float(gamma):.17g}, K_P={self.wasserstein_kernel_lipschitz():.17g}, "
                f"K_sigma={self.noise_metadata['sigma_lipschitz']:.17g})."
            )
        return value
