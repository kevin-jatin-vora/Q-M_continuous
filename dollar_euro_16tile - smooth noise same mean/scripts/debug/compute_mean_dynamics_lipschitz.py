...
def ordinary_artifact(baseline, models, represented, gamma, kernel_lipschitz, noise_metadata):
    output = dict(baseline)
    rows = []
    for row in baseline["constants"]:
        updated = dict(row)
        lf = float(kernel_lipschitz) if int(row["region"]) in represented else 0.0
        updated.update({
            "Lf1": lf, "Lf2": lf, "Lf_sum": lf,
            "LQ_bellman_bound": (
                float(row["Lr_sum"]) * lf / (1.0 - gamma * lf) if lf else 0.0
            ),
            "mean_map_Lf": 1.0 if lf else 0.0,
            "Lf_estimator": "analytic_wasserstein_smooth_scalar_noise",
        })
        rows.append(updated)
    output.update({
        "source": "scripts/debug/compute_mean_dynamics_lipschitz.py",
        "estimator": "analytic_wasserstein_smooth_scalar_noise",
        "Lf_definition": "K_P = sqrt(1 + 2*K_sigma^2) under common-noise coupling",
        "Lf_grouping": "source_category_action",
        "mean_map_Lf": 1.0,
        "operational_dynamics_constant": "wasserstein_kernel_lipschitz",
        "smooth_noise": noise_metadata,
    ...
    output["method_notes"]["Lf"] = (
        "The deterministic mean map has Lf=1. The Lf fields used by the Bellman "
        "bound contain the analytic Wasserstein kernel constant K_P so state-dependent "
        "noise is included without using independently sampled successor ratios."
    )
    return output
...
def main():
    ...
    kernel_lipschitz = float(provenance["wasserstein_kernel_lipschitz"])
    gamma_kernel_lipschitz = gamma * kernel_lipschitz
    if gamma_kernel_lipschitz >= 1.0:
        raise ValueError(
            f"Non-contractive smooth-noise kernel: gamma*K_P={gamma_kernel_lipschitz:.17g} >= 1"
        )
    noise_metadata = {
        key: provenance[key] for key in (
            "noise_model_version", "noise_interpolation", "category_noise_multipliers",
            "tile_noise_multiplier_grid_row_major", "noise_multiplier_lipschitz",
            "sigma_lipschitz", "wasserstein_kernel_lipschitz",
        ...
        for pair_number, pair_info in enumerate(neighbor_pairs):
            ...
            for action in range(4):
                ...
                cross_rows.append({
                    ...
                    "sampled_fitted_mean_map_sensitivity": sampled_lf,
                    "sampled_mean_displacement_difference_norm": jump,
                    "mean_map_Lf": 1.0,
                    "Lf1": kernel_lipschitz, "Lf2": kernel_lipschitz,
                    "Lf_sum": kernel_lipschitz,
                    "Lf_witness": result["witness"],
                })

    baseline = json.loads((baseline_dir / "lipschitz_constants.json").read_text(encoding="utf-8"))
    ordinary = ordinary_artifact(
        baseline, models, represented, gamma, kernel_lipschitz, noise_metadata
    )
    cross = {
        ...
        "gamma_times_wasserstein_kernel_lipschitz": gamma_kernel_lipschitz,
        "interpretation": (
            "Operational Lf is the analytic global Wasserstein kernel bound. "
            "Gap-restricted fitted-mean ratios are retained only as diagnostics."
        ),
    ...
    local_dir = output_root / "wasserstein_kernel_local"
    cross_dir = output_root / "wasserstein_kernel_cross"
    ...
    worst = {action: kernel_lipschitz for action in range(4)}
    for row in cross_rows:
        action = int(row["action"])
        worst[action] = max(worst[action], float(row["Lf_sum"]))
    cross_valid = all(gamma * value < 1.0 for value in worst.values())
    results = {
        "wasserstein_kernel_local": cross_valid,
        "wasserstein_kernel_cross": cross_valid,
        "mean_map_Lf": 1.0,
        "wasserstein_kernel_lipschitz": kernel_lipschitz,
        "gamma": gamma,
    ...
    if not cross_valid:
        ...
        print("wasserstein_kernel_cross is non-contractive; its Q-bound training will be skipped.")
