...
from dollar_euro_lipschitz.env import smooth_noise_metadata
...
def make_run_dir(provenance):
    ...
    tag = (
        ...
        f"det{provenance['determinism']:g}_steps{provenance['steps']}"
    )
    run_id = hashlib.sha256(
        json.dumps(provenance, sort_keys=True, separators=(",", ":")).encode("utf-8")
    ).hexdigest()[:8]
    return ROOT / "outputs" / "diagnostics" / f"{tag}_{run_id}"
...
def resolve_run_dir(forward_args):
    ...
    provenance.update(noise_metadata)
    provenance["gamma_times_wasserstein_kernel_lipschitz"] = (
        float(parsed.gamma) * float(noise_metadata["wasserstein_kernel_lipschitz"])
    )
...
def minimum_represented_pruning_radius(bounds_path):
    bounds = json.loads(Path(bounds_path).read_text(encoding="utf-8"))
    radii = [
        float(row["pruning_radius"])
        for action_rows in bounds["by_region_action"].values()
        for row in action_rows.values()
        if not row.get("unused_category") and int(row.get("n_total", 0)) > 0
    ]
    if not radii:
        raise RuntimeError("Cannot derive pair-distance threshold: no represented radius cells")
    return float(min(radii))


def summarize_wasserstein_variants(gap_dir, variants):
    def format_triplet(row, keys):
        values = [row[key] for key in keys]
        if any(value is None for value in values):
            return "n/a (no neighboring category pairs)"
        return "/".join(f"{value:.12g}" for value in values)

    comparison = []
    for variant in variants:
        variant_dir = gap_dir / variant
        ordinary_path = variant_dir / "lipschitz_constants.json"
        cross_path = variant_dir / "cross_category_dynamics_lipschitz.json"
        if not ordinary_path.is_file() or not cross_path.is_file():
            continue
        ordinary = json.loads(ordinary_path.read_text(encoding="utf-8"))
        cross = json.loads(cross_path.read_text(encoding="utf-8"))
        ordinary_values = [float(row["Lf_sum"]) for row in ordinary["constants"]]
        cross_values = [float(row["Lf_sum"]) for row in cross["constants"]]
        comparison.append({
            "variant": variant,
            "ordinary_min": min(ordinary_values) if ordinary_values else None,
            "ordinary_mean": sum(ordinary_values) / len(ordinary_values) if ordinary_values else None,
            "ordinary_max": max(ordinary_values) if ordinary_values else None,
            "cross_min": min(cross_values) if cross_values else None,
            "cross_mean": sum(cross_values) / len(cross_values) if cross_values else None,
            "cross_max": max(cross_values) if cross_values else None,
        })
    destination = gap_dir / "wasserstein_variant_comparison.json"
    destination.write_text(json.dumps(comparison, indent=2), encoding="utf-8")
    print("\nData-fitted Wasserstein variant comparison")
    print(f"{'variant':<14} {'ordinary min/mean/max':<43} {'cross min/mean/max'}")
    for row in comparison:
        ordinary_text = format_triplet(
            row, ("ordinary_min", "ordinary_mean", "ordinary_max")
        )
        cross_text = format_triplet(row, ("cross_min", "cross_mean", "cross_max"))
        print(f"{row['variant']:<14} {ordinary_text:<43} {cross_text}")
...
def main():
    ...
    if any(
        ...
        raise SystemExit(
            "--min-pair-distance was removed; the diagnostic now uses the minimum "
            "represented pruning radius from transition_bounds.json"
        )
    ...
    required_shared = (
        ...
        shared_dir / "q1_dqn_r1.pth",
        shared_dir / "q2_dqn_r2.pth",
    )
    missing_shared = [str(path.resolve()) for path in required_shared if not path.is_file()]
    if missing_shared:
        raise RuntimeError(
            "Transition collection/reuse left an incomplete shared dataset. Missing: "
            + ", ".join(missing_shared)
        )

    args.min_pair_distance = minimum_represented_pruning_radius(
        shared_dir / "transition_bounds.json"
    )
    print(
    ...
    variants = ["local_max", "global_max", "local_mean", "global_mean"]
    summarize_wasserstein_variants(gap_dir, variants)
    summary = []
