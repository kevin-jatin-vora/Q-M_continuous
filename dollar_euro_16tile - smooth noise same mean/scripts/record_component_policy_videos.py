...
def main():
    ...
    run_config_path = experiment_dir / "data" / "run_config.json"
    ...
    q1 = models_dir / "q1_dqn_r1.pth"
    q2 = models_dir / "q2_dqn_r2.pth"
    if not q1.is_file():
    ...
    print(
        f"sigma={sigma:.17g} determinism={determinism:.17g} "
    ...
    record_policy_video(
        checkpoint=q1,
        output=videos_dir / "r1_policy.mp4",
        environment=environment,
        seed=args.seed,
        fps=args.fps,
        substeps=args.substeps,
        title=f"R1 greedy (sigma={sigma:g}, det={determinism:.0%})",
        device=device,
    )
    record_policy_video(
        checkpoint=q2,
        output=videos_dir / "r2_policy.mp4",
        environment=environment,
        seed=args.seed,
        fps=args.fps,
        substeps=args.substeps,
        title=f"R2 greedy (sigma={sigma:g}, det={determinism:.0%})",
        device=device,
    )
