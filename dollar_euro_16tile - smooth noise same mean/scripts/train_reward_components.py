...
from dollar_euro_lipschitz.env import (
    ContinuousDollarEuroEnv, noise_multiplier_lipschitz, smooth_noise_metadata,
)
...
LC_ESTIMATOR_VERSION = "reward_and_wasserstein_local_max_min_radius_v3"


def minimum_represented_pruning_radius(bounds):
    """Smallest radius among category/action cells that contain collected data."""
    radii = []
    for action_rows in bounds["by_region_action"].values():
        for row in action_rows.values():
            if row.get("unused_category") or int(row.get("n_total", 0)) <= 0:
                continue
            radii.append(float(row["pruning_radius"]))
    if not radii:
        raise ValueError("Cannot derive pair-distance threshold: no represented radius cells")
    return float(min(radii))


def pair_distance_mask(distances, min_pair_distance):
    """Reject zero-distance pairs and enforce the derived minimum radius."""
    distances = np.asarray(distances, dtype=np.float64)
    threshold = float(min_pair_distance)
    if threshold > 1e-12:
        return distances >= threshold
    return distances > 1e-12
...
def train_component(args, environment, reward_index, seed, device):
    ...
    env.validate_wasserstein_contractivity(args.gamma)
...
def reward_lipschitz_per_tile(
    next_tile_records, model, device, max_pairs, seed, min_pair_distance=0.0,
):
    ...
    for tile_id, (states, next_states, rewards) in sorted(next_tile_records.items()):
        ...
        valid = pair_distance_mask(distances_nxt, min_pair_distance)
        ...
        report[tile_id] = {
            "n": n_rows,
            "Lr": float(np.max(np.abs(rewards64[left] - rewards64[right]) / distances_nxt)),
...
def _bilinear_category_basis(states, tile_map):
    ...
    sx = np.clip((states[:, 0] - 0.125) / 0.25, 0.0, 3.0)
    sy = np.clip((states[:, 1] - 0.125) / 0.25, 0.0, 3.0)
    ix, iy = np.minimum(np.floor(sx).astype(np.int64), 2), np.minimum(np.floor(sy).astype(np.int64), 2)
    tx, ty = sx - ix, sy - iy
...
def fit_data_wasserstein_model(records1, records2, tile_map, theoretical_metadata):
    """MLE fit of common action means and bilinear category-center sigmas."""
    all_states, all_residuals, action_means = [], [], []
    for action in range(4):
        chunks = []
        ...
        states = np.concatenate([row[0] for row in chunks], axis=0)
        next_states = np.concatenate([row[1] for row in chunks], axis=0)
        deltas = next_states - states
        mean = deltas.mean(axis=0)
        all_states.append(states)
        all_residuals.append(deltas - mean)
        action_means.append(mean)
    states = np.concatenate(all_states, axis=0)
    residuals = np.concatenate(all_residuals, axis=0)
    energy = np.sum(residuals * residuals, axis=1)
    basis = _bilinear_category_basis(states, tile_map)
    active = np.flatnonzero(basis.sum(axis=0) > 0.0)
    observed_rms = float(np.sqrt(np.mean(energy) / 2.0))
    if float(theoretical_metadata["sigma_lipschitz"]) == 0.0 or observed_rms < 1e-10:
        fitted = np.zeros(5, dtype=np.float64)
        success, message = True, "zero-noise numerical-floor branch"
    else:
        x = basis[:, active]
        def objective(log_scales):
            ...
            return float(value), scales * (x.T @ derivative) / energy.size
        result = minimize(
            objective, np.log(np.full(active.size, max(observed_rms, 1e-12))),
            jac=True, method="L-BFGS-B", bounds=[(-30.0, 1.0)] * active.size,
            options={"maxiter": 200, "ftol": 1e-13},
        )
        fitted = np.zeros(5, dtype=np.float64)
        fitted[active] = np.exp(result.x)
        success, message = bool(result.success), str(result.message)
    fitted_grid = fitted[np.asarray(tile_map, dtype=np.int64).reshape(4, 4) - 1]
    k_sigma = noise_multiplier_lipschitz(fitted_grid)
    return {
        "category_sigma": fitted,
        "category_grid": np.asarray(tile_map, dtype=np.int64).reshape(4, 4),
        "action_mean_displacement": np.asarray(action_means),
        "fit_success": success,
        "fit_message": message,
        "sample_estimated_sigma_lipschitz": k_sigma,
        "sample_estimated_global_K_P": float(np.sqrt(1.0 + 2.0 * k_sigma ** 2)),
    }


def data_wasserstein_constant(
    set_a, set_b, action, fitted_model, max_pairs, seed, min_pair_distance=0.0,
):
    """Maximum fitted Gaussian W2 ratio over sampled source-state pairs.

    Passing the same set gives an ordinary within-category/action constant;
    passing two sets gives a neighboring-category/action constant. This compares
    fitted conditional distributions, not independent successor noise draws.
    """
    states_a, states_b = np.asarray(set_a[0], dtype=np.float64), np.asarray(set_b[0], dtype=np.float64)
    n_a, n_b = states_a.shape[0], states_b.shape[0]
    empty = {"n_a": int(n_a), "n_b": int(n_b), "Lf": 0.0, "pairs_used": 0}
    if n_a == 0 or n_b == 0:
        return empty
    rng = np.random.default_rng(seed + 13 * n_a + 29 * n_b + 41 * action)
    same = set_a[0] is set_b[0]
    total = n_a * (n_a - 1) // 2 if same else n_a * n_b
    if same:
        ia, ib = unique_unordered_pairs(n_a, max_pairs, rng)
    elif max_pairs is None or total <= int(max_pairs):
        ib, ia = np.tile(np.arange(n_b), n_a), np.repeat(np.arange(n_a), n_b)
    else:
        flat = rng.choice(total, size=int(max_pairs), replace=False)
        ia, ib = flat // n_b, flat % n_b
    distance = np.linalg.norm(states_a[ia] - states_b[ib], axis=1)
    valid = pair_distance_mask(distance, min_pair_distance)
    ia, ib, distance = ia[valid], ib[valid], distance[valid]
    if not distance.size:
        return empty
    sigma_a = _bilinear_category_basis(states_a[ia], fitted_model["category_grid"]) @ fitted_model["category_sigma"]
    sigma_b = _bilinear_category_basis(states_b[ib], fitted_model["category_grid"]) @ fitted_model["category_sigma"]
    ratios = np.sqrt(distance ** 2 + 2.0 * (sigma_a - sigma_b) ** 2) / distance
    return {
        "n_a": int(n_a), "n_b": int(n_b),
        "Lf": float(np.max(ratios)), "pairs_used": int(distance.size),
    }


def data_wasserstein_per_category(
    records, fitted_model, max_pairs, seed, min_pair_distance=0.0,
):
    report = {}
    for (category, action), sample_set in sorted(records.items()):
        result = data_wasserstein_constant(
            sample_set, sample_set, action, fitted_model, max_pairs,
            seed + 17 * category + 31 * action, min_pair_distance,
        )
        report[(category, action)] = {
            "n": result["n_a"], "Lf": result["Lf"],
...
def build_lipschitz(
    ...
    - Lf: values supplied by the caller. Production supplies data-fitted local-
      max Wasserstein constants per source category/action; the optional
      ``noise_metadata`` path remains available to analytic comparison callers.
    ...
    """
    # An optional analytic comparison caller may replace all rows with global K_P.
    # Production omits metadata here and supplies its local-max W2 estimates.
    analytic_kernel = noise_metadata is not None
    kernel_lf = (
        float(noise_metadata["wasserstein_kernel_lipschitz"])
        if analytic_kernel else None
    )
    category_lf = {}
    for region in range(1, 6):
        for action in range(4):
            ...
            category_lf[(region, action)] = {
                "Lf1": kernel_lf if analytic_kernel else float(first["Lf"]),
                "Lf2": kernel_lf if analytic_kernel else float(second["Lf"]),
                "Lf_sum": (
                    kernel_lf if analytic_kernel
                    else max(float(first["Lf"]), float(second["Lf"]))
                ),
                "raw_sampled_Lf1": float(first["Lf"]),
                "raw_sampled_Lf2": float(second["Lf"]),
                "n_r1": int(first["n"]),
                "n_r2": int(second["n"]),
            }
            ...
            rows.append(
                {
                    ...
                    "Lf_sum": lf_sum,
                    "mean_map_Lf": 1.0,
                    "raw_sampled_Lf1": lf_row["raw_sampled_Lf1"],
                    "raw_sampled_Lf2": lf_row["raw_sampled_Lf2"],
                    "Lq1": float(eq1["Lq"]),
                    "Lq2": float(eq2["Lq"]),
    ...
    return {
        ...
        **(noise_metadata or {}),
        **(
            {"gamma_times_wasserstein_kernel_lipschitz": args.gamma * kernel_lf}
            if analytic_kernel else {}
        ),
        "Lr_definition": "|R(s1')-R(s2')| / ||s1'-s2'||",
        "Lr_grouping": "next_state_tile",
        "Lr_action_dependence": False,
        "Lf_definition": (
            "analytic W2 kernel bound sqrt(1 + 2*K_sigma^2)"
            if analytic_kernel else "||s1'-s2'|| / ||s1-s2||"
        ),
        "Lf_grouping": (
            "global_smooth_noise_kernel"
            if analytic_kernel else "source_category_action"
        ),
        "Lq_definition": "Lr*Lf/(1-gamma*Lf)",
        "method_notes": {
            "Lr": (
                "Per-NEXT-STATE-tile maximum pairwise ratio on free transitions "
            ...
            "Lf": (
                "Operational Lf is the analytic global Wasserstein K_P. Per-source-"
                "category/action sampled successor ratios are retained only in "
                "raw_sampled_Lf1/raw_sampled_Lf2 audit fields."
                if analytic_kernel else
                "Raw diagnostic Lf is the per-source-category/action sampled "
                "constant supplied by the caller. Production supplies fitted-"
                "Gaussian local-max W2 ratios."
            ),
            "LQ_bellman_bound": (
                "LQ_bellman_bound = Lr(Ti) * Lf(Ci,a) / (1 - gamma * Lf(Ci,a)); "
...
def cross_category_constants(set_a, set_b, model, action, device, max_pairs, seed):
    ...
    return {
        "n_a": n_a,
        "n_b": n_b,
        "Lr": float(np.max(np.abs(rewards_a64[idx_a] - rewards_b64[idx_b]) / distances_nxt)),
...
def cross_tile_reward_constants(
    set_a, set_b, max_pairs, seed, min_pair_distance=0.0,
):
...
def main():
    ...
    noise_metadata = smooth_noise_metadata(sigma, determinism, det_sigma_scale)
    gamma_kp = float(args.gamma) * float(noise_metadata["wasserstein_kernel_lipschitz"])
    if gamma_kp >= 1.0:
        raise ValueError(
            f"Non-contractive smooth-noise kernel: gamma*K_P={gamma_kp:.17g} >= 1"
        )
    print(
        "Smooth-noise contractivity: "
        f"K_sigma={noise_metadata['sigma_lipschitz']:.9g} "
        f"K_P={noise_metadata['wasserstein_kernel_lipschitz']:.9g} "
        f"gamma*K_P={gamma_kp:.9g}"
    )
    ...
    fitted_wasserstein = fit_data_wasserstein_model(
        records1, records2, tile_map, noise_metadata
    )
    fitted_wasserstein_json = {
        "method": "data_fitted_common_mean_and_bilinear_isotropic_gaussian_noise",
        "fit_success": fitted_wasserstein["fit_success"],
        "fit_message": fitted_wasserstein["fit_message"],
        "category_sigma": {
            str(category): float(fitted_wasserstein["category_sigma"][category - 1])
            for category in range(1, 6)
        },
        "action_mean_displacement": fitted_wasserstein["action_mean_displacement"].tolist(),
        "sample_estimated_sigma_lipschitz": fitted_wasserstein["sample_estimated_sigma_lipschitz"],
        "sample_estimated_global_K_P": fitted_wasserstein["sample_estimated_global_K_P"],
        "theoretical_global_K_P_verification": noise_metadata["wasserstein_kernel_lipschitz"],
    }
    print(
        "Data-fitted Wasserstein model: "
        f"sample global K_P={fitted_wasserstein['sample_estimated_global_K_P']:.9g} "
        f"theory global K_P={noise_metadata['wasserstein_kernel_lipschitz']:.9g}"
    )
    print("Operational dynamics Lf estimator: local_max (ordinary and neighboring-category pairs)")
    bounds = build_bounds(records1, records2, args, sigma, category_counts=cat_counts)
    min_pair_distance = minimum_represented_pruning_radius(bounds)
    bounds["metadata"]["lc_pair_distance_threshold"] = min_pair_distance
    bounds["metadata"]["lc_pair_distance_source"] = (
        "minimum pruning_radius over represented category/action cells"
    )
    print(
        "Automatic LC pair-distance threshold: "
        f"min represented pruning radius={min_pair_distance:.9g}"
    )
    ...
    category_dynamics1 = data_wasserstein_per_category(
        records1, fitted_wasserstein, args.max_pairs, args.seed,
        min_pair_distance,
    )
    category_dynamics2 = data_wasserstein_per_category(
        records2, fitted_wasserstein, args.max_pairs, args.seed + 7,
        min_pair_distance,
    )
    ...
    constants["Lf_definition"] = "data-fitted Gaussian W2 ratio"
    constants["Lf_grouping"] = "source_category_action_data_fitted_wasserstein"
    constants["data_fitted_wasserstein_model"] = fitted_wasserstein_json
    constants["theoretical_noise_metadata"] = noise_metadata
    constants["method_notes"]["Lf"] = (
        "Per-source-category/action maximum Gaussian W2 ratio over sampled source "
        "pairs. Common action "
        "means and the bilinear isotropic noise field are fitted from R1/R2 "
        "transition residuals; theoretical global K_P is verification only."
    )
    constants["Lf_estimator"] = "local_max"
    constants["Lr_estimator"] = "local_max"
    constants["lc_estimator_version"] = LC_ESTIMATOR_VERSION
    constants["min_pair_distance"] = min_pair_distance
    ...
    for tile_i, tile_j in neigh_tile_pairs:
        ...
        x1 = cross_tile_reward_constants(
            set_a1, set_b1, args.max_pairs, args.seed + 21, min_pair_distance
        )
        x2 = cross_tile_reward_constants(
            set_a2, set_b2, args.max_pairs, args.seed + 23, min_pair_distance
        )
    ...
    cross_tile_artifact = {
        ...
        "Lr_estimator": "local_max",
        "lc_estimator_version": LC_ESTIMATOR_VERSION,
    ...
    for pair_info in neighbor_cat_pairs:
        ...
        for action in range(4):
            ...
            x1 = data_wasserstein_constant(
                set_a1, set_b1, action, fitted_wasserstein,
                args.max_pairs, args.seed + 31, min_pair_distance
            )
            x2 = data_wasserstein_constant(
                set_a2, set_b2, action, fitted_wasserstein,
                args.max_pairs, args.seed + 33, min_pair_distance
            )
    ...
    cross_cat_artifact = {
        ...
        "deterministic_sigma_scale": det_sigma_scale,
        "deterministic_sigma": det_sigma_scale * sigma,
        "data_fitted_wasserstein_model": fitted_wasserstein_json,
        "theoretical_noise_metadata": noise_metadata,
        "tile_map": tile_map.tolist(),
        "Lf_estimator": "local_max",
        "lc_estimator_version": LC_ESTIMATOR_VERSION,
        "min_pair_distance": min_pair_distance,
        "min_pair_distance_source": "minimum represented pruning radius",
        "max_pairs": int(args.max_pairs),
        ...
        "Lf_grouping": "neighbor_source_category_pair_data_fitted_wasserstein",
        "lipschitz_method_version": 3,
    ...
    manifest = {
        ...
        "deterministic_sigma": det_sigma_scale * sigma,
        **noise_metadata,
        "gamma_times_wasserstein_kernel_lipschitz": gamma_kp,
        "data_fitted_wasserstein_model": fitted_wasserstein_json,
        "lf_estimator": "local_max",
        "lr_estimator": "local_max",
        "lc_estimator_version": LC_ESTIMATOR_VERSION,
        ...
        "cross_tile_reward_lipschitz": {
            ...
            "methodology": (
                "Maximum pairwise NEXT-state-tile reward ratios "
        ...
        "cross_category_dynamics_lipschitz": {
            ...
            "methodology": (
                "Maximum data-fitted Gaussian W2 ratio across sampled pairs from neighboring "
                "source categories; theoretical global K_P is verification only."
            ),
        },
        "noise_action_ratio": {
