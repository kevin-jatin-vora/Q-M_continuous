...
def verify_runtime_sigma(config, sigma: float) -> dict:
    ...
    category_multipliers = {1: 1.0, 2: 1.1, 3: 1.2, 4: 1.3}
    category_variances = {}
    for category, multiplier in category_multipliers.items():
        covariance = np.asarray(
            env.terrain_config["regions"][category]["noise_cov"], dtype=np.float64
        )
        expected_var = (multiplier * float(sigma)) ** 2
        expected_covariance = np.eye(2, dtype=np.float64) * expected_var
        tolerance = max(1e-30, abs(expected_var) * 1e-9)
        if np.any(np.abs(covariance - expected_covariance) > tolerance):
            raise SystemExit(
                f"error: category-{category} covariance is {covariance.tolist()}, "
                f"expected ({multiplier}*sigma)^2={expected_var:.17g} on the diagonal"
            )
        category_variances[category] = expected_var
    cat1_var = category_variances[1]
    cat4_var = category_variances[4]
    ...
    return {
        ...
        "category_noise_multipliers": category_multipliers,
...
def main():
    ...
    identity = (
        ...
        + b"\0lc_estimator_version="
        + LC_ESTIMATOR_VERSION.encode("ascii")
    )
    if args.steps is not None:
        identity += (
            b"\0effective_steps="
