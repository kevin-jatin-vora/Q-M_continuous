...
From project root `dollar_euro_16tile - smooth noise same mean`.
...
- **Lr** is grouped by NEXT-state tile (action-pooled). Operational **Lf** comes
  from the data-fitted Gaussian Wasserstein model: per category/action ordinarily
  and per neighboring-category/action across boundaries. The analytic global
  `K_P` is retained as a verification and preflight contractivity value.
...
- Theoretical RA fails if `γ·Lf ≥ 1` (raises a clear error, no substitution /
  never returns 0).
- `sigma=0`: noise is exactly zero and `K_P=1`. With nonzero sigma,
  `determinism=1` uses the category-5 multiplier `deterministic_sigma_scale`.
